...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['finalizar'])) {
        ...
        if ($produto) {
            ...
            $sql_check = "SELECT * FROM estoque WHERE nome_produto = ? AND id_turma = ?";
            $stmt_check = $conn->prepare($sql_check);
            $stmt_check->bind_param("si", $produto['nome_produto'], $turma);
            $stmt_check->execute();
            $result_check = $stmt_check->get_result();
            $existing_product = $result_check->fetch_assoc();

            if ($existing_product) {
                // Verifica se a posição é diferente
                if ($existing_product['posicao'] != $produto['posicao']) {
...
header('Location: ../movimentarD3.php', true, 301);
exit;
